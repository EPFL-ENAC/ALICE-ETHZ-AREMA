export default {
  main: {
    brand: 'AREMA',
    brand_caption: 'Base de données de matériaux régénératifs',
  },
  login: {
    submit: 'Soumettre',
    register: 'S\'inscrire',
    email: 'Courriel',
    password: 'Mot de passe',
    registerSuccess: 'Inscription réussie',
    registerError: 'Échec de l\'inscription',
  },
  locales: {
    en: 'English',
    fr: 'Français',
  },
  profile: {
    current: 'Mon profil',
    current_hint: 'Les informations de votre profil sont visibles publiquement en tant qu\'auteur.',
    identifier: 'Identifiant',
    type: 'Type',
    name: "Nom affiché",
    affiliation: 'Affiliation',
    affiliation_hint: 'Votre organisation ou affiliation.',
    description: 'Description',
    description_hint: 'Une brève biographie ou description de vous-même.',
    email: 'Courriel public',
    email_hint: 'Ce courriel sera visible publiquement dans votre profil.',
    web: 'Site web',
    update_success: 'Profil mis à jour avec succès',
    not_found: 'Profil non trouvé',
  },
  roles: {
    administrator: 'Administrateur',
    contributor: 'Contributeur',
    reviewer: 'Réviseur',
  },
  states: {
    draft: 'Brouillon',
    'in-review': 'En révision',
    'to-publish': 'À publier',
    'to-unpublish': 'À dépublier',
    'to-delete': 'À supprimer',
    locked: 'Verrouillé',
  },
  importer: {
    import: 'Importer',
    iglehm: {
      project_import: {
        title: 'Import d\'un projet IG Lehm',
        hint: 'Sélectionnez un projet IG Lehm. Cela créera un nouvel article avec le même titre que le projet IG Lehm et un contenu contenant toutes les informations du projet.',
      },
      specialist_import: {
        title: 'Import d\'un spécialiste IG Lehm',
        hint: 'Sélectionnez un spécialiste IG Lehm. Cela créera un nouvel article avec le même nom que le spécialiste IG Lehm et un contenu contenant toutes les informations du spécialiste.',
      },
    },
    all: 'Tout',
    new: 'Nouveau',
    update: 'Mise à jour',
    regions: 'Régions',
    sectors: 'Secteurs',
    fields: 'Domaines',
  },
  error: {
    accept_terms_and_conditions: 'Vous devez accepter les termes et conditions.',
    form_invalid: 'Certains champs sont invalides.',
    invalid_email: 'Une adresse courriel valide est requise.',
    registration_failed: "L'inscription a échoué. Veuillez réessayer plus tard.",
    password_complexity_not_met: "Le mot de passe ne répond pas aux exigences de complexité.",
    unknown: "Une erreur inattendue s'est produite.",
  },
  'content-manager': 'Gestionnaire de contenu',
  'content-reviewer': 'Réviseur de contenu',
  absorption_coefficient_hint: 'α',
  absorption_coefficient: "Coefficient d'absorption (à 500Hz)",
  acoustic: 'Acoustique',
  action: 'Action',
  add: 'Ajouter',
  address_input_hint: 'Taper Entrée pour rechercher des adresses.',
  address: 'Adresse',
  admin: 'Administrateur',
  administration: 'Administration',
  administrator: 'Administrateur',
  aerial: 'Aérien',
  air_tightness_hint: 'δ',
  air_tightness: 'Étanchéité à l\'air',
  all_items_indexed: 'Tous les éléments sont republiés ({count}).',
  amount: 'Quantité',
  apply: 'Appliquer',
  areaDelivery: 'Zone de livraison',
  article_bottom: 'Article (bas)',
  article_top: 'Article (haut)',
  assigned_to: 'Assigné à',
  authors: 'Auteurs',
  authors_hint: 'Auteurs/contributeurs de l\'article.',
  authors_synced_from_users: 'Auteurs synchronisés depuis les utilisateurs ({count})',
  building_building_material_used_hint: 'Matériaux de construction utilisés.',
  building_element_building_material_distance_hint:
    'Distance pour obtenir les matériaux de construction en kilomètres.',
  building_element_building_material_used_hint:
    'Matériaux de construction utilisés dans la technique de construction sélectionnée.',
  building_element_building_material_weight_hint:
    'Poids du matériau de construction en kilogrammes.',
  building_element_materials_hint:
    'Fournir des détails sur les matériaux utilisés dans cet élément de bâtiment.',
  building_element_materials: 'Matériaux utilisés',
  building_element_professionals_hint: 'Professionnels impliqués dans la construction.',
  building_element_technical_construction_used_hint: 'Techniques de construction utilisées.',
  building_elements: 'Éléments de bâtiment',
  building_material_constituants_hint:
    'Les matériaux de construction sont fabriqués à partir de ressources naturelles.',
  building_material_constituants: 'Matériaux de construction constituants',
  building_material: 'Matériau de construction',
  building_materials: 'Matériaux de construction',
  building_professionals_hint: 'Professionnels impliqués dans la construction.',
  building_technical_construction_used_hint: 'Techniques de construction utilisées.',
  buildings: 'Bâtiments',
  cancel: 'Annuler',
  discard: 'Abandonner',
  unsaved_changes: 'Modifications non enregistrées',
  unsaved_changes_confirm: 'Vous avez des modifications non enregistrées. Les abandonner et fermer le formulaire ?',
  class: 'Classe',
  classic: 'Classique',
  client: 'Client',
  close: 'Fermer',
  compressive_strength_hint: 'σc, MPa',
  compressive_strength: 'Résistance à la compression',
  confirm_remove: 'Êtes-vous sûr de vouloir supprimer l\'élément "{name}"?',
  constituants: 'Constituants',
  content: 'Contenu',
  contributions: 'Contributions',
  copyright_confirmation: 'Je confirme ici que j’ai les droits de publier toutes les données de ce formulaire, y compris les photos.',
  created_by: 'Ajouté par',
  created_by_me: 'Ajouté par moi',
  credits: 'Crédits',
  dashboard: 'Tableau de bord',
  decorate: 'Décorer',
  decorate_hint: 'Enrichir le texte avec des entrées de terminologie.',
  density_hint: 'ρ, kg/m³',
  density: 'Densité',
  description: 'Description',
  diff: 'Diff',
  diffusivity_hint: 'D, *10⁻⁶*m²/s',
  diffusivity: 'Diffusivité',
  dimension: 'Dimension',
  distance: 'Distance',
  drop_index: 'Supprimer l\'index',
  edit: 'Editer',
  effusivity_hint: 'E, W*sqrt(h)/(m²*K)',
  effusivity: 'Effusivité',
  email: 'Courriel',
  enabled: 'Activé',
  error_loading_authors: "Erreur lors du chargement des auteurs.",
  error_not_found: "Oups. Il n'y a rien ici...",
  external_links: 'Liens externes',
  field_required: 'Champ requis',
  files: 'Fichiers',
  fire_resistance_class_hint: 'DIN EN 13501-1',
  fire_resistance_class: 'Classe de résistance au feu',
  fire_resistance: 'Résistance au feu',
  fire: 'Feu',
  first_name: 'Prénom',
  general: 'Général',
  gross_internal_area_hint: 'En m²',
  gross_internal_area: 'Surface brute intérieure',
  group_by: 'Grouper par',
  guest: 'Invité',
  help: 'Aide',
  high: 'Haut',
  hygrothermal: 'Hygrothermique',
  inactive: 'Inactif',
  index_all: 'Tout republier',
  index_drop_error: "Erreur lors de la suppression de l'index",
  index_dropped: 'Index supprimé',
  guidelines: 'Directives',
  last_modification: 'Dernière modification',
  last_name: 'Nom de famille',
  layout_help:
    "Ceci est un exemple de disposition de page de chaque élément permettant de visualiser comment chaque champ est placé dans la page correspondante du portail web d'AREMA.",
  layout: "Disposition d'un article",
  legend: 'Légende',
  lock_unlock: 'Verrouiller/Déverrouiller',
  water_content: 'Teneur en eau',
  water_content_hint: 'w80%, kg/m³',
  free_saturation: 'Saturation libre',
  free_saturation_hint: 'wf, kg/m³',
  liquid_transfer_coefficient_hint: 'A, kg/(m²*sqrt(s))',
  liquid_transfer_coefficient: 'Coefficient de transfert de liquide pour absorption',
  location: 'Emplacement',
  low: 'Bas',
  markdown_guide: 'Guide Markdown',
  materials: 'Matériaux',
  moisture_buffering_hint: 'MBV, g/(m²*%RH)',
  moisture_buffering: "Valeur tampon de l'humidité",
  multimedia: 'Multimédia',
  name: 'Nom',
  natural_resource_constituants: 'Ressources constituantes',
  natural_resources: 'Ressources',
  no_authors_found: "Aucun auteur trouvé.",
  no_building_element_materials: "Pas d'information sur les matériaux",
  no_building_elements: 'Aucun élément de bâtiment',
  no_results: 'Aucun résultat',
  not_assigned: 'Non assigné',
  not_published: 'Non publié',
  one_file_required: 'Au moins un document multimédia est requis.',
  other_addresses: 'Autres adresses',
  other: 'Autre',
  others: 'Autres',
  password_copied: 'Mot de passe copié dans le presse-papiers',
  password_edit_hint: 'Laisser vide pour NE PAS modifier le mot de passe.',
  password_hint:
    'Le mot de passe doit comporter au moins 8 caractères et contenir un mélange de lettres (majuscules et minuscules), de chiffres et de caractères spéciaux.',
  password_temp_hint:
    "Mot de passe temporaire que l'utilisateur mettra à jour lors de la prochaine connexion. Il doit comporter au moins 8 caractères et contenir un mélange de lettres (majuscules et minuscules), de chiffres et de caractères spéciaux.",
  password: 'Mot de passe',
  phone: 'Téléphone',
  physical_characteristics: 'Caractéristiques physiques',
  porosity_hint: 'ξ en %vol',
  porosity: 'Porosité',
  preview: 'Aperçu',
  professional_building_material_expertise_hint: 'Expertise en matériaux de construction.',
  professional_professionals_hint: 'Professionnels associés.',
  professional_technical_construction_expertise_hint: 'Expertise en techniques de construction.',
  professionals: 'Professionnels',
  properties: 'Propriétés',
  publish: 'Publier',
  publication_flow: 'Flot de publication',
  published_on: 'Publié le {date}',
  published_by: 'Publié par',
  published: 'Publié',
  radius: 'Rayon',
  reaction_to_fire_hint: 'DIN EN 13501-2',
  reaction_to_fire: 'Réaction au feu',
  relations: 'Relations',
  remove_selected: 'Supprimer les items sélectionnés',
  remove_user_text: "Êtes-vous sûr de vouloir supprimer l'utilisateur '{name}'?",
  remove_user: "Supprimer l'utilisateur",
  remove: 'Supprimer',
  reset_password: 'Réinitialiser le mot de passe',
  role: 'Rôle',
  roles_hint:
    'Un contributeur peut ajouter et modifier ses propres éléments, un réviseur peut modifier tous les éléments mais ne peut pas les publier, et un administrateur peut effectuer toutes les opérations, y compris la gestion des utilisateurs.',
  save: 'Sauvegarder',
  search_index_help:
    "L'index de recherche permet de rechercher des éléments par texte libre et par tags. Il s'agit d'une fonctionnalité essentielle du portail web principal d'AREMA. Chaque élément est automatiquement indexé, dès sa création ou sa mise à jour ; votre intervention n'est normalement pas nécessaire. Néanmoins, pour des besoins d'administration avancés, il est possible de supprimer l'index à tout moment, puis de réindexer tous les éléments.",
  search_index: 'Index de recherche',
  settlement_hint: 'mm/m',
  settlement: 'Tassement',
  shrinkage_hint: 'mm/m',
  shrinkage: 'Retrait',
  side_note: 'Note de page',
  signout: 'Déconnexion',
  signin: 'Connexion',
  signup: "S'inscrire",
  signup_successful: 'Inscription réussie',
  snapshot: 'Snapshot',
  snapshot_help: 'Télécharger un snapshot des données actuellement publiées pour référence ultérieure.',
  snapshot_download: 'Télécharger le snapshot',
  snapshot_download_error: 'Erreur lors du téléchargement du snapshot',
  sound_reduction_index_hint: 'Rw, dB',
  sound_reduction_index: 'Indice de réduction sonore pondéré',
  status: 'Etat',
  state: 'Etat',
  std: 'Standard',
  structural: 'Structurel',
  sync_from_users: 'Synchroniser depuis les utilisateurs',
  technical_construction_building_material_constituants_hint:
    'Les techniques de construction sont fabriquées à partir de matériaux de construction.',
  technical_construction_natural_resource_constituants_hint:
    'Les techniques de construction sont fabriquées à partir de ressources naturelles.',
  technical_construction: 'Technique de construction',
  technical_constructions: 'Techniques de construction',
  tensile_strength_hint: 'σ, MPa',
  tensile_strength: 'Résistance à la traction',
  terms_and_conditions: 'Conditions générales',
  terms_and_conditions_accept: "J'accepte les conditions générales",
  terms_and_conditions_reminder: 'Notez que en sauvegardant, vous restez auteur de votre article mais donnez à ETH Zürich les droits de le publier sous la licence CC BY-SA 4.0.',
  terms_and_conditions_show: 'Voir les conditions générales',
  thermal_capacity_hint: 'c, J(kg*K)',
  thermal_capacity: 'Capacité thermique',
  thermal_conductivity_hint: 'λ, W/(m*K)',
  thermal_conductivity: 'Conductivité thermique',
  type: 'Type',
  types: 'Types',
  u_hint: 'U, W/(m²*K)',
  u: 'U',
  unpublish: 'Dépublier',
  updated_by: 'Mis à jour par',
  upload_files_hint: 'Téléverser une ou plusieurs images, vidéos ou fichiers PDF.',
  upload_files: 'Téléverser des fichiers',
  url_hint: "URL d'une image, d'une vidéo ou d'un fichier PDF. Taper 'Entrée' pour valider.",
  url: 'URL',
  user: 'Utilisateur',
  users: 'Utilisateurs',
  vapor_diffusion_resistance_hint: 'µ',
  vapor_diffusion_resistance: "Resistance à la diffusion de vapeur d'eau",
  view: 'Voir',
  website: 'Site web',
  weight: 'Poids',
  write: 'Ecrire',
  year_of_construction: 'Année de construction',
  youngs_modulus_hint: 'E, GPa',
  youngs_modulus: 'Module de Young',
  zone: 'Zone',
};
